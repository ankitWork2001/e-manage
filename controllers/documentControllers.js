...
export const getEmployeeDocuments = async (req, res) => {
  try {
    ...
    if (!doc) {
      return res.status(404).json({ message: 'No documents found for this employee.' });
    }
...
export const deleteDocumentByKey = async (req, res) => {
  try {
    ...
    if (!doc || !doc.documents.has(key)) {
      return res.status(404).json({ message: 'Document not found for given key.' });
    }
...
export const updateDocumentByKey = async (req, res) => {
  try {
    ...
    const doc = await EmployeeDocuments.findOne({ employeeId });
    if (!doc || !doc.documents.has(key)) {
      return res.status(404).json({ message: `Document with key '${key}' not found.` });
    }
